...
\begin{po}{\textbf{等长公理}}\label{po:4}
    \mbox{}\\
    \indent \textbf{5.1.} 给定两点$A,B$以及直线$l$，对$l$上任一点$C$，$C$两侧各有唯一一点$D$，使得线段$|AB| = |CD|$。\\
...
\chapter{平面的公理}
本书中提到，古人研究平面形的性质的过程中，产生了最早的\textbf{公理体系}。

公理体系从几个最基本的假设和定义出发，认为这些假设和定义是不证自明的。依此推导对象的各种性质。
我们在正文中提到了$8$条公理。从它们出发，推出了直线、线段、三角形等基本形状的基本性质。不过，严谨来说，要构建完整的平面形状的公理体系，还需要一些别的公理。它们要么过于基础，我们常常直接默认为显然的事实，要么过于抽象，依赖枯燥的定义和叙述，所以我们没有在正文里介绍。默认它们是真的，并不会对我们今后的学习有过多妨碍。学有余力的读者，可以适当浏览以下关于其他公理的内容，作为补充知识。要强调的是，这些内容并不要求读者掌握或熟知。

下面我们分组来引入剩余的公理。

首先是定义点线面的公理：
\begin{po}{\textbf{点线公理}}\label{po:1-0}
    任何直线至少包含两个不同的点。
\end{po}
\begin{po}{\textbf{线面公理}}\label{po:1-1}
    平面中至少有三点不在同一直线上。
\end{po}
这两条公理非常基础也非常符合直观。它们是用来区分点和直线、线和平面的，避免我们讨论的全体对象来来去去都只是同一个点，或只是同一条直线。

接下来是一组关于顺序的公理。我们在正文中直接使用了点在直线两侧、直线把平面分为两边的说法，但还没有明确说明这是什么意思。下面的公理给出了符合直观理解的定义：
\begin{df}{\textbf{介于关系}}\label{df:a-0}
    我们定义同一直线上相异的三点$A,B,C$之间有一种关系，叫做\textbf{介于关系}，记为$\jie{A}{B}{C}$，读作“$B$介于$A$、$C$之间”。规定$\jie{A}{B}{C}$当且仅当$\jie{C}{B}{A}$。
\end{df}
对于这样的关系。我们有以下公理：
\begin{po}{\textbf{介点公理}}\label{po:1-2}
    给定直线上任意三点$A,B,C$，恰有一点介于另两点之间。换句话说，$\jie{A}{B}{C}$、$\jie{B}{C}{A}$、$\jie{C}{A}{B}$恰有一个成立。
\end{po}
\begin{po}{\textbf{线段公理}}\label{po:1-3}
    对于任一直线上两点$A,B$，总存在直线上的点$C$、$D$使得$\jie{A}{B}{C}$、$\jie{A}{D}{B}$成立。
\end{po}
我们说使得$\jie{A}{B}{C}$的点$C$在线段$AB$的延长线上，$D$在线段$AB$上或在$AB$内。

再接下来是一个规定直线和三角形的基本位置关系的公理。
\begin{po}{\textbf{穿三角公理}}\label{po:1-4}
    设$A,B,C$不共线，且都不属于直线$l$。如果$l$与线段$AB$有交点（存在一点$P\in l$且$jie{A}{P}{B}$）,那么$l$要么与线段$AC$有交点，要么与$BC$有交点（两者必居其一）。
\end{po}
这个公理声明了一个显而易见的性质：直线交三角形某条边进入三角形之后，必然要从另一条边出来。也就是说，如果不考虑直线过三角形顶点的情况，那么直线与三角形的边相交，必然是交于两条边，不会仅仅交于一条边或三条边。

根据以上的公理，我们可以严格定义并讨论线段与射线。线段$AB$是使得$\jie{A}{X}{B}$成立的所有点$X$的集合。射线$AB$是使得$\jie{A}{B}{X}$、$\jie{A}{X}{B}$至少有一个成立的点$X$的集合。

给定直线$l$上一点$O$，首先取$l$中另一点$A$，根据线段公理，有点$B$使得$\jie{A}{O}{B}$。对于$l$中任两点$X,Y$，我们定义：
\begin{enumerate}
    \item $X,Y$在$O$同侧，当且仅当$\jie{X}{O}{Y}$不成立（但三点还是共线）。
    \item $X,Y$在$O$异侧，当且仅当$\jie{X}{O}{Y}$成立。
\end{enumerate}
可以证明“同侧”是一个等价关系，把$l$中除了$O$以外的点分为两个等价类，同类的点在$O$同侧。这就规定了点把直线分出的两侧。实践中为了方便，我们会根据直观图形的位置情况，分别称为点$O$的左右侧或上下侧。每一侧都可以看作由$O$出发的一条射线。

对于直线和平面的关系，我们定义：给定直线$l$，如果$l$以外的两点$A,B$连成的线段和$l$不相交，就说它们在$l$（划分平面）的同侧，否则说它们在$l$的异侧。可以证明，这样定义的“同侧”也是一个等价关系，把平面上$l$以外的点分为两个等价类，称为$l$将平面划分的两侧。

以上和顺序相关的公理还可以推出一个非常基本但重要的结论：
\begin{tm}{\textbf{顺序传递定理}}\label{tm:a-0}
    共线的四点$A,B,C,D$如果使得$\jie{A}{B}{C}$、$\jie{B}{C}{D}$成立，那么$\jie{A}{B}{D}$、$\jie{A}{C}{D}$也成立。
\end{tm}
这个结论直观上显而易见，但从思理的角度，从以上公理出发，需要经过比较复杂的推理得到。这里我们不展开论证。

最后我们引入连续公理。连续公理是用来说明平面中的线连续不断的，把我们直觉上察知的“连续不断”这个性质用公理表述：
\begin{po}{\textbf{连续公理}}\label{po:1-5}
    设直线$l$被分为两个子集$S,T$，使得给定$T$中每一点，$S$中所有点都在其同侧，反之亦然。为了叙述方便，不妨说$S$中所有点都在$T$中任一点左侧，$T$中所有点都在$S$中任一点右侧。那么恰有一点$O\in l$，使得以下两个说法恰有一个为真：
    \begin{enumerate}
        \item $O\in S$且$S$中其他所有点都在$O$左侧。
        \item $O\in T$且$T$中其他所有点都在$O$右侧。
    \end{enumerate}
    $O$称为$S,T$的分割点。
\end{po}
连续公理的意思是：我们把直线分割成左右两半时，总能找到具体的那个分割的地方（用一点来表示）。这样，当我们说两直线相交，把各自分为两侧时，就能具体说有一个交点。

\begin{sk}
    \mbox{} \\
    \indent 1. 用简单易懂的话说明顺序传递定理。最好用纸笔画出直观的位置关系。\\
    \indent 2. 
\end{sk}
